      ...
      <section className="section-space">
        <div className="page-wrap">
          ...
          <div className="grid md:grid-cols-3 gap-10 mt-16">
            {[[ShieldCheck,'Wallet ownership','Only the registered wallet can act for a unit.'],[FileSearch,'Evidence integrity','Duplicate incident references and unreadable evidence are guarded.'],[Handshake,'Bounded settlement','The ruling is proposed first, appealed once, then finalized exactly once.']].map(([Icon,t,d],i)=>{const C=Icon as typeof ShieldCheck;return <Reveal key={String(t)} delay={i*.08}><C size={28}/><h3 className="display-font text-3xl mt-5 mb-3">{String(t)}</h3><p className="text-[var(--ink-soft)] leading-7">{String(d)}</p><div className="mt-6 flex items-center gap-2 text-sm font-bold text-[var(--evergreen)]"><CheckCircle2 size={16}/> Contract V2 rule</div></Reveal>})}
          </div>
        </div>
      </section>
    </>
