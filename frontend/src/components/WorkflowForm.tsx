...
const copy: Record<Mode, { title: string; button: string; functionName: string }> = {
  ...
  close: { title: "Close the response window", button: "Close response window", functionName: "close_response_window" },
...
export function WorkflowForm({ mode, reportId }: { mode: Mode; reportId?: string }) {
  ...
  const args = useMemo(() => {
    if (mode === "join") return [values.unit, BigInt(values.bond || "0")];
    if (mode === "bond") return [BigInt(values.member || "0"), BigInt(values.bond || "0")];
  ...
  async function submit(event: React.FormEvent) {
    ...
    if (!contract) { setState({ tone: "error", message: "NeighborPeace V2 has not been deployed yet. Add the new contract address after Studio deployment." }); return; }
    setState({ tone: "busy", message: "Confirm in your wallet. GenLayer consensus may take a few moments." });
    const result = await writeContract(config.functionName, args, contract);
    if (result.success) setState({ tone: "success", message: String(result.data ?? result.status ?? "Finalized"), hash: result.hash });
  ...
  return (
    <form className="form-panel" onSubmit={submit}>
      ...
      {mode === "join" && <><Field label="Unit name" hint="Use the official unit identifier used by your community."><input className="input" value={values.unit} onChange={(e) => update("unit", e.target.value)} placeholder="Apartment 4B" required /></Field><Field label="Initial bond units" hint="Minimum 50. This is an on-chain ledger value, not a token transfer."><input className="input" type="number" min="50" value={values.bond} onChange={(e) => update("bond", e.target.value)} required /></Field></>}
      {mode === "bond" && <><Field label="Your member ID"><input className="input" type="number" min="0" value={values.member} onChange={(e) => update("member", e.target.value)} required /></Field><Field label="Bond units to add" hint="Only the registered owner wallet can update this member ledger."><input className="input" type="number" min="1" value={values.bond} onChange={(e) => update("bond", e.target.value)} required /></Field></>}
