...
export function OnchainList({ kind }: { kind: "members" | "reports" }) {
  ...
  const [message, setMessage] = useState(contract ? "Loading contract state..." : "NeighborPeace V2 is awaiting deployment.");
  ...
  async function sync() {
    ...
    if (loaded.error) setMessage(loaded.error);
      ...
      setMessage(loaded.total === 0 ? `No ${kind} have been recorded on Contract V2 yet.` : `Synced ${loaded.records.length} ${kind} from Studionet.`);
    }
    setBusy(false);
  }
